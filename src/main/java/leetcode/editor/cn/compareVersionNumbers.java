...
class Solution {
    public int compareVersion(String version1, String version2) {
        ...
        int v1N=version1S.length;
        int v2N=version2S.length;
        int n = Math.max(v1N, v2N);
        int i=0;
        for( i=0;i<n;i++){
            Integer v1 = getNum(version1S,i);
            Integer v2 = getNum(version2S,i);
            if(v1<v2){
                return -1;
            }else if(v1>v2){
                return 1;
    ...
    private int getNum(String[] strings,int k){
        int n=strings.length;
        if(k<n){
            return Integer.valueOf(strings[k]);
        }else {
            return 0;
        }
    }
